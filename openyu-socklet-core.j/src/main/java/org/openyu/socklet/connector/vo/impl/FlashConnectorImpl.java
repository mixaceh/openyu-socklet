...
public class FlashConnectorImpl extends SocketConnectorSupporter implements FlashConnector {
	...
	private static final transient Logger LOGGER = LoggerFactory.getLogger(FlashConnectorImpl.class);

	// // flash -> server, length=22
	// private static final String FLASH_INPUT_POLICY =
	// "<policy-file-request/>";
	//
	// private static final byte[] FLASH_INPUT_POLICY_BYTES =
	// ByteHelper.toBytes(FLASH_INPUT_POLICY);
	//
	// // server -> flash, length=87
	// private static final String FLASH_OUTPUT_POLICY =
	// "<cross-domain-policy><allow-access-from domain=\"*\"
	// to-ports=\"*\"/></cross-domain-policy>";
	//
	// private static final byte[] FLASH_OUTPUT_POLICY_BYTES =
	// ByteHelper.toBytes(FLASH_OUTPUT_POLICY);
	//
	// // length=1,flash socket,xmlSocket用
	//// private static final char FLASH_EOF = (char) 0x00;
	////
	//// private static final byte[] FLASH_EOF_BYTES =
	// ByteHelper.toBytes(FLASH_EOF);
	//
	...
	public FlashConnectorImpl(String id, Class moduleTypeClass, Class messageTypeClass, ProtocolService protocolService,
			String ip, int port) {
	...
	protected void readable(SelectionKey selectionKey) throws Exception {
		...
		// read handshake, 36
		// ---------------------------------------------
		else if (!readHandshake && sendHandshake && !handshake && bytes.length != 88 && bytes.length != 34
				&& bytes.length >= 36) {
	...
	protected void receiveMessage(byte[] bytes) {
		// 接收者
		if (receiver == null) {
			LOGGER.warn("[" + id + "] " + "no receiver to receive message");
			return;
		}
		//
		bytes = ByteHelper.getByteArray(bytes, 0, bytes.length - FLASH_EOF_BYTES.length);
