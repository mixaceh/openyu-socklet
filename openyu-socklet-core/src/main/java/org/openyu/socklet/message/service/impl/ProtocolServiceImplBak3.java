 ...
 * 組成/反組byte[]
 */
public abstract class ProtocolServiceImplBak3 extends BaseServiceSupporter implements
		ProtocolService {

	private static final long serialVersionUID = 7562819271771546706L;

	private static transient final Logger LOGGER = LoggerFactory
			.getLogger(ProtocolServiceImplBak3.class);
	...
	public ProtocolServiceImplBak3() {
	}
	...
	public Message dehandshake(byte[] values) {
		...
		try {
			...
			// 實際收的資料長度
			AssertHelper.isTrue(values.length >= 38, "Data length ["
					+ values.length + "] is invalid");

			// 實際收的資料長度 <資料總長度
			AssertHelper.isTrue(values.length >= totalLength, "Data length ["
					+ values.length + "] < expected length [" + totalLength
					+ "] bytes");
